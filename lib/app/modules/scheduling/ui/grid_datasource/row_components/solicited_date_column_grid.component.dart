...
class SolicitedDateColumnGrid extends StatefulWidget {
  ...
  const SolicitedDateColumnGrid({required this.schedulingDataEntity, required this.updateSchedulingItem, Key? key}) : super(key: key);

  @override
  _SolicitedDateColumnGridState createState() => _SolicitedDateColumnGridState();
}

class _SolicitedDateColumnGridState extends State<SolicitedDateColumnGrid> {
  ...
  @override
  void initState() {
    selectedDate = widget.schedulingDataEntity.dateSolicitedInDateTime;
    super.initState();
  }

  @override
  void didUpdateWidget(covariant SolicitedDateColumnGrid oldWidget) {
    selectedDate = widget.schedulingDataEntity.dateSolicitedInDateTime;
  ...
  void onChangedDate(DateTime value) async {
    ...
    widget.schedulingDataEntity.dateSolicited = value.toString();
    Either<SchedulingError, bool> response = await widget.updateSchedulingItem(widget.schedulingDataEntity);
    if (response.isRight) {
      setState(() {
        selectedDate = value;
      });
    }else{
      widget.schedulingDataEntity.dateSolicited = selectedDate.toString();
    }
